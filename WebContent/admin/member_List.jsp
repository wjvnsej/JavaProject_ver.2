...
List<MemberDTO> bbs = dao.selectListPage(param);
			...
			<!-- 리스트반복 start -->
				<tr>
					...
					<td class="text-center" style="overflow:hidden; text-overflow: ellipsis;  white-space: nowrap;">
						<a href="member_View.jsp?id=<%=dto.getId() %>&nowPage=<%=nowPage %>&<%=queryStr %>">
							<%=dto.getName() %>
						</a>
					</td>
					<td class="text-center"><%=dto.getPhone() %></td>				
					<td class="text-center"><%=dto.getJoin_date() %></td>
					<td class="text-center"><%=dto.getGrade() %></td>
					<td class="text-center">
						<button type="button" class="btn btn-outline-warning" 
							onclick="location.href='member_Edit.jsp?grade=<%=grade %>&id=<%=dto.getId() %>';">회원수정</button>
